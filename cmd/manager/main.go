...
func printVersion() {
	...
	log.Info(fmt.Sprintf("Operator Commit SHA: %s", version.CommitSHA))
...
func main() {
	...
	err = leader.Become(ctx, "multitenancy-lock")
	if err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
	...
	if webhookEnabled {
		log.Info("Setting up webhook server handlers")
		webhook.SetupServerHandlers(mgr, webhookCertDir)
	} else {
		log.Info("Webhook server is disabled")
	}

	// Setup the eventhook client set
	// controller-runtime client does not contain all needed features of the kubernetes API
	// so we manage a seperate clientset for the event hooks
	if err := eventhooks.RegisterConfig(cfg); err != nil {
		log.Error(err, "Failed to setup event hook clientset")
		os.Exit(1)
	}
